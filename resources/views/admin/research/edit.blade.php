...
@section('content')
    <form action="/admin/research/{{ $activity->id }}" method="POST" enctype="multipart/form-data" id="researchForm">
        @csrf
        @method('PUT')
        <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" class="form-control @error('type') is-invalid @enderror">
                            ...
                            @foreach($types as $type)
                                <option value="{{ $type }}"
                                    {{ old('type', $activity->type) === $type ? 'selected' : '' }}>{{ $type }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="department" class="form-control">
                            ...
                            @foreach($departments as $dept)
                                <option value="{{ $dept }}"
                                    {{ old('department', $activity->department) === $dept ? 'selected' : '' }}>{{ $dept }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group">
                <label>Authors</label>
                <input type="text" name="authors" value="{{ old('authors', $activity->authors) }}" class="form-control">
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="url" name="link" value="{{ old('link', $activity->link) }}" class="form-control">
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                @if($activity->image)
                    <div class="mb-2">
                        <img src="{{ $activity->image }}" style="max-width:200px;" alt="current image">
